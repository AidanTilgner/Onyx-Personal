...
def silero_stt(language="en", version="latest", jit_model="jit", **kwargs):
    ...
    model, decoder = init_jit_model(
        model_url=models.stt_models.get(language).get(version).get(jit_model), **kwargs
    )
...
def silero_tts(language="en", speaker="kseniya_16khz", **kwargs):
    ...
    assert (
        speaker in available_speakers
    ), f"Speaker not in the supported list {available_speakers}"
    assert (
        language == speaker_language[speaker]
    ), f"Incorrect language '{language}' for this speaker, please specify '{speaker_language[speaker]}'"

    model_conf = models.tts_models[language][speaker].latest
    if "_v2" in speaker or "_v3" in speaker:
...
def silero_te():
    ...
    return (model, example_texts, languages, punct, apply_te)
